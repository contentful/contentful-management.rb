...
`vcr` (with `webmock` as the HTTP adapter) was adopted from the initial commit. Cassette YAML files live in `spec/fixtures/`. Tests record against the real CMA API once; subsequent runs replay from cassettes.
